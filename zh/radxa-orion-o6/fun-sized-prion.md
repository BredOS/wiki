...
| RTC                                                 | 工作    | 潜行者                                                                                                                                                       |
| UART                                                | 工作    | 开机时`ttyS2`                                                                                                                                                |
| PCIe (任何 M.2 端口) | 部分的   | Works fine for most devices but some GPUs don't work as expected and freezes the entire system sometimes. See **5. PCIe** |
| M.2M 键                              | 工作    |                                                                                                                                                           |
| M.2 E 键                             | 工作    |                                                                                                                                                           |
| M.2 B 键                             | 工作    |                                                                                                                                                           |
| 风扇控制                                                | Works | 自动粉丝控制，不能控制操作系统                                                                                                                                           |

# 🔄 3. PCIe 稳定性

一些测试者发现，当使用PCIe Gen4速度运行的设备连接时，该系统变得不稳定。 一些测试者发现，当使用PCIe Gen4速度运行的设备连接时，该系统变得不稳定。 如果你的棋盘不稳定，请考虑更新到我们的 UEFI 固件并设置链接速度到 3 基因。 一些测试者发现，当使用PCIe Gen4速度运行的设备连接时，该系统变得不稳定。 如果你的棋盘不稳定，请考虑更新到我们的 UEFI 固件并设置链接速度到 3 基因。 If your board is unstable due to this, consider setting the link speed to Gen. 3.