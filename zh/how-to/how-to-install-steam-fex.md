...
> 本文是针对单一板载计算机（SBC），它以ARMv9系统-on-Chip（SOC）为基础，例如Cix Pi 81x0。 ARMv8 SOC支持执行32位代码。 对于ARMv8 SBC，请使用框64代替！
> {.is-info}
...
确保您已经在您的主机系统上安装了专有的 NVIDIA 驱动程序。 开源的 Nouveau 驱动程序将无法工作。

- 要在 FEX-Container 中安装x86_64 NVidia 驱动程序，请运行：

```
sudo bash ./cp-nvidia-driver.sh
```

> 此脚本可以在我们早些时候克隆的 FEX-Emu 仓库中找到。
> {.is-info}
