...
> 本指南涵盖完全开源堆栈(Rocket + Teflon)。 将Rockchip专有的 RKNN Toolkit2 改为我们的 [RKNN](/en/NPU/RKNN)。
> {.is-info}
...
所有使用 RK3588`或`RK3588S\` SoC 支持的 BredOS 支持的板块以及对主线的支持都可以使用 NPU。 This includes the Rock 5B, Rock 5B Plus, Orange Pi 5 series, and others listed on the [supported devices](/en/table-of-supported-devices) page.
...
> 如果`/dev/grad/redded0`不存在，您的内核可能会超过`6.18`或缺少`CONFIG_DRM_ACCEL_ROCKET` 选项。 更新到最近的 BredOS 内核并查看[7。 疑难解答](#h-7-Troubleshooting)。
> {.is-info}
...
- **没有高级活动** - 像SiLU这样的操作 (在 YOLOv8中使用) 尚未实现。
- **单核执行** - 虽然RK3588拥有3个NPU核心，但当前驱动程序每次只使用一个核心。
- **CNN焦点** - 堆栈被优化用于复杂神经网络。 基于转换的模型不会加速。
